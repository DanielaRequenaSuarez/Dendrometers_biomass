...
setwd("C:/Users/Daniela Requena/Google Drive/ECI_dendrometers/Codes") 
dir()
...
cen1  <- subset(census, plotname=="BOB-01")
cen   <- subset(cen1, year==census_year)  
dend1 <- subset(dendrometer, plot_code=="BOB-01")
...
colnames(npp_tree) <- c("plot_code", "tag", "year", "month", "agC", "agCdiff", "datediff")
  ...
  nppacw_cen  <- NPPacw_census(census, plotname="BOB-01", allometric_option="Default", height_correction_option="Default", census1_year=2013, census2_year=2015)
...
plot(www$month, www$nppacw_month)##### NPP ACW (MgC ha-1 mo-1)
